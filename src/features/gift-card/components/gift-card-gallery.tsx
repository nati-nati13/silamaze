          ...
          <div className="grid grid-cols-2 gap-4">
            {GIFT_CARD_GALLERY_ITEMS.map((item) => (
              <div key={item.id} className="flex flex-col gap-2">
                <div
                  role="img"
                  aria-label={`${item.label} — რეალური ვიზუალი მალე დაემატება`}
                  className="flex aspect-square items-center justify-center rounded-2xl border border-border bg-muted"
                >
